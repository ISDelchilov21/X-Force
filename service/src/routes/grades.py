...
from db.db_grade import create_grade, get_report, get_user_report, update_grade, get_grade_homework, create_statistics
...
async def create_grades(grade_im: GradeHomeworkIM, homework_id:int, token: Annotated[str, Depends(oauth2_scheme)]):
    payload = jwt.decode(token, public_key, algorithms=["RS256"])
    user_role: str = payload["user_role"]
    if user_role == "teacher" or user_role == "admin":

        
        grade = create_grade(grade_im.grade, grade_im.report, homework_id, grade_im.user_id)
...
async def get_report_user(grade_id:int, grade_im:UpdateGradeHomeworkIM,  token: Annotated[str, Depends(oauth2_scheme)]):
    ...
    if user_role == "teacher" or user_role == "admin":
        ...
        grade = update_grade(grade_im.id, grade_im.grade, grade_im.report, grade_im.homework_id)
        if not grade:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Homeworks are not found",
