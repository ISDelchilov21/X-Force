...
@router.get("/get/class/by/name/{class_name}", tags=["class"])
...
@router.get("/get/class/by/id/{class_id}", tags=["class"])
...
@router.get("/get/students/class/{class_id}", tags=["class"])
...
@router.get("/get/teachers/class/{class_id}", tags=["class"])
...
@router.post("/create/class/{org_id}", tags=["class"])
async def create_class(
    class_im: ClassesIM, org_id: int, token: Annotated[str, Depends(oauth2_scheme)]
):
...
@router.post("/join/class", tags=["class"])
async def user_join_class(
    class_join_im: ClassJoinIM, token: Annotated[str, Depends(oauth2_scheme)]
):
    ...
    class_im = bridgeClassUserIM(
        user_id=id,
        user_role=user_role,
        class_id=get_class["id"],
        class_name=get_class["name"],
        class_subject=get_class["subject"],
    )

    org = add_user_to_class(
        class_im.user_id,
        class_im.user_role,
        class_im.class_id,
        class_im.class_name,
        class_im.class_subject,
    )

    return org


@router.post("/assign-student/class/{class_id}", tags=["class"])
async def assign_student_in_class(
    class_im: bridgeClassUserIM,
    class_id: int,
...
@router.post("/assign-teacher/class/{class_id}", tags=["class"])
async def assign_teacher_in_class(
    class_im: bridgeClassUserIM,
    class_id: int,
    token: Annotated[str, Depends(oauth2_scheme)],
):
...
@router.delete("remove-student/{user_id}/classes/{class_id}", tags=["class"])
async def remove_student_from_class(
    class_id: int, user_id: int, token: Annotated[str, Depends(oauth2_scheme)]
):
    payload = jwt.decode(token, public_key, algorithms=["RS256"])
    user_role: str = payload["user_role"]
...
@router.delete("remove-teacher/{user_id}/classes/{class_id}", tags=["class"])
