...
from db.db_homeworks import (
    create_homework,
    get_homework_by_title,
    get_homework_by_id,
    add_homework_to_class,
    add_homework_to_user,
    get_homework_in_class,
    get_homeworks,
    get_homeworks_in_class,
    get_user_homework,
    delete_homework_by_id,
    delete_homework_by_title,
    update_homework,
    user_submit_homework,
    get_submited_homeworks,
    create_attachment,
    add_link_to_attachment,
    get_homeworks_in_class,
)
...
@router.get("/get/homework/by/title/{homework_title}", tags=["homeworks"])
...
@router.get("/get/homework/by/id/{homework_id}", tags=["homeworks"])
...
def get_homework_from_class(class_id: int):
...
def getr_homework_use(user_id: int):
...
@router.get("/get/submited/homeworks/{user_id}", tags=["homeworks"])
async def get_submited_user_homework(token: Annotated[str, Depends(oauth2_scheme)]):
...
async def create_homeworks(
    hm_im: HomeworkIM, class_id: int, token: Annotated[str, Depends(oauth2_scheme)]
):
    payload = jwt.decode(token, public_key, algorithms=["RS256"])
    user_role: str = payload["user_role"]
